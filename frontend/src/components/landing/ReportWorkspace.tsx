...
export default function ReportWorkspace() {
  ...
  return (
    <div className="space-y-10">
      <div className="app-container flex flex-col gap-6 lg:flex-row" data-animate="zoom" id="report">
        <div ref={mapRef} className="min-h-[60vh] flex-1 rounded-3xl border border-white/10 shadow-2xl" style={{ zIndex: 0 }} />
        <aside className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-900/70 p-6 text-slate-50 shadow-2xl backdrop-blur-2xl">
      ...
      <section className="rounded-3xl border border-white/10 bg-slate-900/70 p-8 text-slate-100 shadow-2xl backdrop-blur-2xl" data-animate="rise">
        <header className="mb-6 border-b border-white/5 pb-4">
          ...
          <h3 className="text-2xl font-semibold text-white">Live feed of what people are patching</h3>
        </header>
        {issues.length === 0 ? (
          <p className="text-sm text-slate-300">
          ...
          <ul className="flex max-h-80 flex-col gap-4 overflow-y-auto pr-2">
            {issues.map((issue) => (
              <li key={issue.id} className="rounded-2xl border border-white/5 bg-slate-950/40 p-4 shadow-inner">
